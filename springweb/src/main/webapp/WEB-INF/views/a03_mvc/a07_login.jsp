...
*/
	var id = "${member.id}" // 로그인 화면에 입력한 아이디값 할당
	var sessId = "${mem.id}" // DB로 인증한 모델데이터 id값 할당
	if(id!="") { // 초기화면과 구분처리 : id, pwd를 입력 후 로그인 처리 되었을때
		if(sessId!=""){ // 로그인으로 데이터가 DB에 있어서 인증이 되었을때
			alert("로그인 성공\n메인페이지로 이동")
									...
									<form class="user" method="post">
										...
										<div class="form-group">
											<input type="password" name="pwd" class="form-control form-control-user"
												id="exampleInputPassword" placeholder="패스워드입력">
										</div>
										<div class="form-group">
